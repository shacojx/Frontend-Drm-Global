...
export function HomePage() {
  ...
  return (
    <div className="w-screen h-screen bg-cover flex flex-col overflow-hidden">
      <PageLayoutLeftSideTab
        tabOptions={Object.values(TabOptionGroup)}
        onClickTabOption={handleChangeTab}
        tabIdSelected={homeContent}
        openCallerRef={openCallerRef}
      >
        <div className={"w-full h-full flex flex-col"}>
          <div
            className={
              "w-full h-20 shrink-0 bg-white flex justify-between lg:justify-end items-center px-6"
            }
          >
            <IconThreeLines
              className={"block lg:hidden w-5 h-5 cursor-pointer"}
              onClick={openCallerRef.current}
            />
            <IconAccountCircle
              className={"w-10 h-10 cursor-pointer"}
              onClick={setIsShowAccountPopup.bind(undefined, (value) => !value)}
            />
          </div>
          <div className={"w-full flex grow relative overflow-y-scroll"}>
            {isShowAccountPopup && (
              <div
                ref={ref}
                className={
                  "absolute z-10 top-3 right-8 flex flex-col gap-3 items-center bg-[#E9EEF6] rounded-3xl p-3"
                }
              >
                <div
                  onClick={setIsShowAccountPopup.bind(undefined, false)}
                  className={
                    "absolute top-2 right-2 p-2 bg-gray-300 rounded-full cursor-pointer"
                  }
                >
                  <IconX />
                </div>
                ...
                <div
                  onClick={handleClickAccountOnPopUp}
                  className={
                    "flex flex-row gap-2 w-[290px] bg-white px-6 py-4 mt-3 rounded-xl cursor-pointer"
                  }
                >
                  <IconUser />
                ...
                <div
                  ...
                  className={
                    "flex flex-row gap-2 w-[290px] bg-white px-6 py-4 rounded-xl cursor-pointer"
                  }
                >
                  <IconLogout />
                  <span className={"font-bold"}>
            ...
            {homeContent === TabOptionGroup.myServices.id && (
              <MyServicesContent key={TabOptionGroup.myServices.id} />
            )}
            ...
            {homeContent === "myAccount" && (
              <MyAccountContent
                onClickVerifyKYC={setHomeContent.bind(undefined, "KYCUpload")}
                key="KYCUpload"
              />
            )}
            {homeContent === "KYCUpload" && (
              <KYCUploadContent
                backToMyAccount={setHomeContent.bind(undefined, "myAccount")}
                key="myAccount"
              />
            )}
          </div>
...
function ServicesContent() {
  ...
  const [bunchOfServiceIdSelected, setBunchOfServiceIdSelected] = useState<
    string[]
  >([]);
    ...
    <div className={"w-full grow flex flex-col"}>
      <div className={"flex p-3 grow overflow-hidden"}>
        {stepIndex === SelectServiceStepIndex && (
          <div
            className={
              "p-6 bg-white rounded grow overflow-y-scroll overflow-x-hidden space-y-8"
            }
          >
            {user?.companyType && (
              <div className={"text-cXl w-full text-start"}>
                {translation.t("Since you launch your new in", {
                  companyType: user.companyType,
                })}
                <span className={"text-cLg font-bold text-primary"}>
                  {nationName}
                </span>{" "}
        ...
        {stepIndex === PayServiceStepIndex && (
          <div
            className={
              "flex flex-col sm:flex-row grow gap-3 overflow-y-scroll  overflow-x-hidden"
            }
          >
            <div
              className={
            ...
            >
              <div className={"p-6 grow flex gap-8 flex-col"}>
                <div className={"flex flex-col gap-y-2 w-fit"}>
                  <p className={"text-cXl font-bold"}>
                    {translation.t("Order summary")}
                  </p>
              ...
              <div
                className={
                  "bg-primary_light p-6 font-bold flex justify-between items-center"
                }
              >
            ...
            <div className={"sm:w-1/2 bg-white grow shrink-0 rounded-lg flex"}>
              <div className={"p-6 grow flex flex-col"}>
                <div className={"flex flex-col gap-y-2 w-fit"}>
                  <p className={"text-cXl font-bold"}>
                    {translation.t("Please select a payment method")}
                  </p>
                  <div className={"h-[2px] bg-black w-1/2"}></div>
                </div>
...
function ServiceCard(props: ServiceCardProps) {
  return (
    <div
      className={
        "relative pl-8 sm:pl-14 w-full flex cursor-pointer border rounded-2xl " +
    ...
    >
      ...
      <div className={"flex flex-col grow py-6 gap-3"}>
        ...
        <div>
          <div>
            ...
            <ul
              className={
                "flex flex-col sm:flex-row gap-2 sm:gap-12 list-disc text-violet pl-8"
              }
            >
      ...
      <div
        className={"p-6 text-orange flex items-center justify-center shrink-0"}
      >
  ...
  return (
    <div className={"w-full grow flex flex-col p-3"}>
      ...
      >
        ...
        <p>
          {translation.t(
            "Manage your information, privacy and security so DRMGlobal works for you"
          )}
          .
        </p>
...
function GeneralInformationForm() {
  ...
  const [phone, setPhone] = useState<RNPhoneValue | undefined>(
    generatePhone(
      user?.codePhone || "+84",
      user?.phone.slice(user?.codePhone?.length) || ""
    )
  );
    ...
    <>
      <div className={"flex flex-row justify-between mb-8"}>
        ...
        <div className={"flex flex-row gap-3 items-center"}>
          ...
          {user?.kycStatus === "Pending" && (
            <div
              className={
                "flex flex-row gap-1 items-center bg-[#5D50C626] p-2 rounded-lg"
              }
            >
          ...
          {user?.kycStatus === "In-progress" && (
            <div
              className={
                "flex flex-row gap-1 items-center bg-[#FF572240] p-2 rounded-lg"
              }
            >
              <IconRefreshCircle className={"shrink-0 text-black w-5 h-5"} />
              <span className={"font-bold"}>
                {translation.t("In-progress")}
              </span>
            </div>
          )}
          {user?.kycStatus === "Approved" && (
            <div
              className={
                "flex flex-row gap-1 items-center bg-success p-2 rounded-lg"
              }
            >
              <IconCheck className={"shrink-0 text-white w-5 h-5"} />
              <span className={"font-bold text-white"}>
                {translation.t("Approved")}
              </span>
            </div>
          )}
        </div>
      ...
      <div className={"space-y-6 grow"}>
        {user?.kycStatus === "Pending" && (
          <div
            className={
              "flex flex-row gap-4 items-center p-2 bg-red-200 rounded-lg"
            }
          >
            <IconDangerCircle className={"shrink-0 text-danger"} />
            <p>
              {translation.t(
                "You have not verified your account. Please verify for the best experience"
              )}
              .
            </p>
          </div>
  ...
  return (
    <>
      <div className={"w-full grow flex flex-col p-3"}>
        <div
          className={
            "flex flex-col grow overflow-x-hidden overflow-y-scroll bg-white rounded justify-start items-center py-6 px-4"
          }
        >
          <div className={"w-full max-w-[800px] flex flex-col gap-y-6"}>
            <div className={"flex flex-row justify-between"}>
              ...
              <div className={"flex flex-row gap-3 items-center"}>
                ...
                {user?.kycStatus === "Pending" && (
                  ...
                  >
                    <IconDangerCircle
                      className={"shrink-0 text-black w-5 h-5"}
                    />
                    <span className={"font-bold"}>
            ...
            <p className={"font-bold"}>
              1. {translation.t("Upload your Passport")}
            </p>
            <TakeOrUploadPhoto onUpload={setFile} />
            <p className={"font-bold"}>
              2. {translation.t("Upload your picture holding the passport")}
            </p>
            <TakeOrUploadPhoto onUpload={setFile2} />
            <div className={"flex flex-row justify-end gap-4"}>
              <button
                className={
                  "flex justify-center items-center gap-2 font-semibold rounded-lg py-4 px-6 border text-gray-600"
                }
              ...
              <button
                ...
                className={
                  "py-4 px-6 flex flex-row justify-center items-center gap-2 text-white font-semibold rounded-lg " +
                  (isDisableSend ? " bg-primary_25" : " bg-primary")
                }
              >
  ...
  return (
    ...
    >
      ...
      <ul className={"list-disc flex flex-col items-center"}>
        ...
        <li>
          {translation.t(
            "The photo is in color and should be a valid file (PDF)"
          )}
        </li>
