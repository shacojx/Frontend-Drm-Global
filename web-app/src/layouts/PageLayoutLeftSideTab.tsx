...
export function PageLayoutLeftSideTab<T extends string>(props: Props<T>) {
  ...
  const [isOpenOnSmallScreen, setIsOpenOnSmallScreen] =
  ...
  return (
    <div className={"w-full h-full bg-surface flex flex-row"}>
      {isOpenOnSmallScreen && (
        <div
          className={
            "lg:none bg-black absolute z-50 top-0 left-0 w-screen h-screen opacity-40"
          }
        ></div>
      )}
      <div
      ...
      >
        <div className={"grow"}>
          <div
            className={
              "flex flex-row items-center mx-4 mt-6 justify-between lg:justify-start"
            }
          >
            <img
            ...
            <div
              className={
                "block lg:hidden p-2 bg-gray-100 rounded-full cursor-pointer"
              }
            >
...
function TabOptionItem<T>(props: TabOptionProps<T>) {
  const translation = useTranslation();
  return (
    <Menu.Items className="absolute w-[-156px] h-[50px] left-auto -right-[219px] top-[50px] lg:top-0 mt-2 bg-white border border-gray-200 rounded shadow-lg z-10">
      <Menu.Item>
        {({ active }) => (
          <Menu.Button
            onClick={props.onClick.bind(undefined, props.id)}
