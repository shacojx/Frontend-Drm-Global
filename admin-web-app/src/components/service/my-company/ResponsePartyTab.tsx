...
export function ResponsePartyTab({
  readonly,
  responseParty,
  onChange,
}: ResponsePartyTabProps) {
  const { t } = useTranslation();
  const { validateCaller } = useValidateCaller();

  const handleFormChange = <K extends keyof ResponseParty>(
    key: K,
    value: ResponseParty[K],
  ) => {
      ...
      <div className="flex flex-col justify-between xl:col-span-1 col-span-2">
        ...
        >
          ...
          <FormFieldText
            isRequired
            className={cn('w-full', {
              'hidden xl:invisible': !responseParty?.hasSSNorITIN,
            })}
            id="SSN/ITIN"
